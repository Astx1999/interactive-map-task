...
const setMobxPersistStore = (target, persistStore) => {
    if (process.env.NODE_ENV !== 'production') {
        for (const [key, store] of PersistStoreMap_1.PersistStoreMap.entries()) {
            if (store.storageName === persistStore.storageName) {
                store.stopPersisting();
                PersistStoreMap_1.PersistStoreMap.delete(key);
            }
        }
    }
    PersistStoreMap_1.PersistStoreMap.set(target, persistStore);
};
